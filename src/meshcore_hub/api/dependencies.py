...
def _build_mqtt_config(app: FastAPI) -> MQTTConfig:
    ...
    mqtt_username = getattr(app.state, "mqtt_username", None)
    mqtt_password = getattr(app.state, "mqtt_password", None)
    mqtt_prefix = getattr(app.state, "mqtt_prefix", "meshcore")
    mqtt_tls = getattr(app.state, "mqtt_tls", False)
    mqtt_transport = getattr(app.state, "mqtt_transport", "tcp")
    mqtt_ws_path = getattr(app.state, "mqtt_ws_path", "/mqtt")
